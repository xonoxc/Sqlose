...
export function pullImage(dbType: DBType): AsyncAppResult<void> {
   ...
   return withTimeout(
      ...
      .catch((e: Error) =>
         err(new DockerError("docker:pull_failed", e.message ?? `Failed to pull image ${config.image}`)),
      )
}

export function waitForDatabaseReady(dbType: DBType, connectionString: string): AsyncAppResult<void> {
...
export function createEnvironment(dbType: DBType): AsyncAppResult<{
   ...
   return findAvailablePort().then((portResult) => {
         ...
         if (!reservePort(port)) {
            return err(new DockerError("docker:port_conflict", `Port ${port} is already reserved`)) as EnvResult
         }
               ...
               docker
                  ...
                  .then((container) =>
                     withTimeout(
                        ...
                        return waitForDatabaseReady(dbType, connectionString).then((readyResult) => {
                           if (readyResult.isErr()) {
                              container.stop().catch(() => Promise.resolve())
                              container.remove({ v: true, force: true }).catch(() => Promise.resolve())
                              releasePort(port)
