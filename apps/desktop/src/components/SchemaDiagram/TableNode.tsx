...
export function TableNode({ data }: { data: TableNodeData }) {
   ...
   return (
      <div
         className="w-72 rounded-[10px] shadow-[0_8px_30px_rgb(0,0,0,0.4)] transition-all duration-300 font-sans border flex flex-col group animate-in fade-in zoom-in-95 relative overflow-visible"
         style={{
      ...
      >
         <div
            className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none duration-500 z-30 rounded-[10px]"
            style={{
         ...
         <div className="flex flex-col py-2 px-1 relative z-20 bg-transparent">
            {data.columns.map(col => (
               ...
               >
               {/* Target handle: left side — FK columns receive incoming edges */}
               <Handle
                  type="target"
                  position={Position.Left}
                  id={`target-${col.name}`}
                  className="!w-2 !h-2 !rounded-full !opacity-0 hover:!opacity-100 !transition-opacity !-left-1"
                  style={{
               ...
               {/* Source handle: right side — PK/columns send outgoing edges */}
               <Handle
                  type="source"
                  position={Position.Right}
                  id={`source-${col.name}`}
                  className="!w-2 !h-2 !rounded-full !opacity-0 hover:!opacity-100 !transition-opacity !-right-1"
                  style={{
                     backgroundColor: currentTheme.colors.accent,
                     border: "none",
                  }}
