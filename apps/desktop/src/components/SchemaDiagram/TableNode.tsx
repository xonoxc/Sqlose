   ...
   return (
      ...
      >
         ...
         <div 
            ...
            style={{ boxShadow: `inset 0 0 20px ${currentTheme.colors.accent}10, 0 0 0 1px ${currentTheme.colors.accent}20` }} 
         />
         ...
         >
            <div className="flex items-center gap-2">
                <IconTable className="h-4 w-4 opacity-70" style={{ color: currentTheme.colors.text }} />
                <span 
            ...
            {data.columns.map(col => (
               ...
               >
                  <Handle 
                      ...
                      style={{ backgroundColor: currentTheme.colors.text, border: 'none', left: -5, right: 'auto' }} 
                  />
                  
                  <div className="flex items-center gap-2 min-w-0 flex-1">
                     {col.primaryKey ? (
                        <IconKey className="h-3 w-3 shrink-0" style={{ color: currentTheme.colors.warning, opacity: 0.8 }} />
                     ) : col.name.endsWith("_id") || col.name.includes("id_") ? (
                        <IconLink className="h-3 w-3 shrink-0" style={{ color: currentTheme.colors.textMuted, opacity: 0.6 }} />
                     ) : (
                        <div className="h-3 w-3 shrink-0 flex items-center justify-center" />
                     )}
                     <span
                        className="text-[12.5px] truncate"
                        style={{ color: currentTheme.colors.text, fontWeight: col.primaryKey ? 500 : 400, opacity: col.primaryKey ? 0.9 : 0.8 }}
                     >
                        {col.name}
                     </span>
                  </div>
                  ...
                  <Handle 
                      ...
                      id={`source-${col.name}`}
                      className="w-1.5 h-3 rounded-sm opacity-0 hover:opacity-100 transition-opacity" 
                      style={{ backgroundColor: currentTheme.colors.text, border: 'none', right: -5, left: 'auto' }} 
                  />
               </div>
            ))}
         </div>
