      ...
      return (
         <div className="flex items-center justify-center h-full bg-bg-primary">
            <div className="flex flex-col items-center justify-center gap-3 text-text-muted">
               ...
               <span className="text-[14px] font-medium">Executing query...</span>
            </div>
         </div>
      )
   }
   ...
   if (error) {
      return (
         ...
         >
            ...
            <div>
               <p className="text-[15px] font-semibold text-error mb-2">Query Execution Failed</p>
               <p className="text-[14px] text-text-secondary font-mono bg-bg-secondary p-3 rounded-md border border-border/50 max-w-4xl whitespace-pre-wrap">
                  {error}
               </p>
            </div>
         </motion.div>
...
function MessagesTab({
   ...
   if (isExecuting) {
      return (
         <div className="flex items-center justify-center h-full">
            <span className="text-[13px] text-text-muted">Query is running...</span>
         </div>
      )
   }
      ...
      return (
         <div className="p-4">
            <div className="p-3 rounded bg-error/5 border border-error/20">
               <p className="text-[13px] font-medium text-error mb-1">Error</p>
               <pre className="text-[12px] text-text-secondary font-mono whitespace-pre-wrap">
                  {error}
               </pre>
            </div>
         </div>
   ...
   if (!result) {
      return (
         <div className="flex items-center justify-center h-full">
            <span className="text-[13px] text-text-muted/60">No messages</span>
         </div>
      )
   }
   ...
   return (
      <div className="p-4">
         <div className="p-3 rounded bg-success/5 border border-success/20">
            <p className="text-[13px] font-medium text-success">Query executed successfully</p>
...
function StatsTab({
   ...
   if (!result && !error) {
      return (
         <div className="flex items-center justify-center h-full">
            <span className="text-[13px] text-text-muted/60">Run a query to see statistics</span>
         </div>
      )
   }
            ...
            <div className="mt-3 p-3 rounded bg-bg-secondary border border-border/50">
               ...
               <div className="flex flex-wrap gap-1.5">
                  {result.columns.map(col => (
                     <span
                        key={col}
                        className="text-[11px] font-mono text-text-secondary bg-bg-tertiary px-1.5 py-0.5 rounded border border-border/40"
                     >
                        {col}
                     </span>
                  ))}
...
function PlanTab() {
   return (
      <div className="flex flex-col items-center justify-center h-full text-text-muted/60">
         ...
         <span className="text-[13px]">Query plan requires EXPLAIN support</span>
         <span className="text-[11px] mt-1">Run a query prefixed with EXPLAIN to see the plan</span>
      </div>
   )
}
