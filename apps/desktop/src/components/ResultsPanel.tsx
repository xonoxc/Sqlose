...
import { IconAlertCircle, IconLoader2, IconDatabaseOff, IconTable, IconInfoCircle, IconChartBar, IconFileCode } from "@tabler/icons-react"
...
export function ResultsPanel({ result, error, isExecuting, executionTimeMs, rowCount }: ResultsPanelProps) {
   ...
   const tabContent = () => {
      switch (activeTab) {
         ...
         case "stats":
            return <StatsTab result={result} error={error} isExecuting={isExecuting} executionTimeMs={executionTimeMs} rowCount={rowCount} />
         case "plan":
            return <PlanTab />
      }
   }
...
function ResultsTab({ result, error, isExecuting }: {
   result: QueryResult | null
   error: string | null
   isExecuting: boolean
}) {
            ...
            <div className="flex flex-col items-center justify-center gap-3 text-text-muted/60 opacity-80">
               ...
               <div className="flex flex-col gap-1.5 mt-3">
                  <div className="flex items-center gap-2 text-[10px] font-mono bg-bg-secondary px-2.5 py-1 rounded border border-border/50">
                     <kbd className="bg-bg-primary px-1 py-0.5 rounded border border-border/40 text-[9px]">⌘</kbd>
                     <kbd className="bg-bg-primary px-1 py-0.5 rounded border border-border/40 text-[9px]">↵</kbd>
                     <span className="text-text-muted">Run query</span>
                  </div>
                  <div className="flex items-center gap-2 text-[10px] font-mono bg-bg-secondary px-2.5 py-1 rounded border border-border/50">
                     <kbd className="bg-bg-primary px-1 py-0.5 rounded border border-border/40 text-[9px]">⌘</kbd>
                     <kbd className="bg-bg-primary px-1 py-0.5 rounded border border-border/40 text-[9px]">K</kbd>
...
function MessagesTab({ result, error, isExecuting }: {
   result: QueryResult | null
   error: string | null
   isExecuting: boolean
}) {
      ...
      return (
         <div className="p-4">
            <div className="p-3 rounded bg-error/5 border border-error/20">
               ...
               <pre className="text-[11px] text-text-secondary font-mono whitespace-pre-wrap">{error}</pre>
            </div>
         </div>
      )
   }
   ...
   return (
      <div className="p-4">
         <div className="p-3 rounded bg-success/5 border border-success/20">
            ...
            <p className="text-[11px] text-text-secondary mt-1">Returned {result.rowCount} row{result.rowCount !== 1 ? "s" : ""} in {result.executionTimeMs}ms</p>
         </div>
      </div>
   )
}

function StatsTab({ result, error, isExecuting, executionTimeMs, rowCount }: {
   result: QueryResult | null
   error: string | null
   isExecuting: boolean
   executionTimeMs: number | null
   ...
   const stats = [
      { label: "Status", value: error ? "Failed" : "Success", color: error ? "text-error" : "text-success" },
         ...
         <div className="grid grid-cols-2 gap-3">
            {stats.map((s) => (
               <div key={s.label} className="p-3 rounded bg-bg-secondary border border-border/50">
                  <p className="text-[10px] font-semibold tracking-wide text-text-muted uppercase mb-1">{s.label}</p>
                  <p className={cn("text-[13px] font-mono font-medium", s.color || "text-text-primary")}>{s.value}</p>
               </div>
            ))}
         </div>
         {result && (
            <div className="mt-3 p-3 rounded bg-bg-secondary border border-border/50">
               <p className="text-[10px] font-semibold tracking-wide text-text-muted uppercase mb-1">Columns</p>
               <div className="flex flex-wrap gap-1.5">
                  {result.columns.map((col) => (
                     <span key={col} className="text-[10px] font-mono text-text-secondary bg-bg-tertiary px-1.5 py-0.5 rounded border border-border/40">
                        {col}
                     </span>
                  ))}
               </div>
