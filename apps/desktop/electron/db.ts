...
export function saveQuery(
   ...
   db!.run(
      "INSERT INTO saved_queries (id, name, sql_text, tags, environment_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
      [id, name, sql, JSON.stringify(tags), environmentId, now, now]
   )
   persist()
}
...
export function updateSavedQuery(id: string, name: string, sql: string, tags: string[]): boolean {
   ...
   const now = new Date().toISOString()
   db!.run(
      "UPDATE saved_queries SET name = ?, sql_text = ?, tags = ?, updated_at = ? WHERE id = ?",
      [name, sql, JSON.stringify(tags), now, id]
   )
   persist()
   return true
}
...
export function addHistoryEntry(
   ...
   db!.run(
      "INSERT INTO history (id, sql_text, environment_id, db_type, duration, row_count, status, error_text, executed_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
      [id, sql, environmentId, dbType, duration, rowCount, status, error, executedAt]
   )
   persist()
}
